...
public class JobPostActivityController {
    ...
    public String searchJobs(Model model, @RequestParam(value = "job", required = false) String job,
        ...
        if (officeOnly == null && remoteOnly == null && partialRemote == null){
            officeOnly ="Office-Day";
            remoteOnly = "Remote-Only";
            partialRemote = "Partial-Remote";
            type = false;
        }

        //if we don't have any item selected, get all jobs
        if(!dateSearchFlag && !remote && !type && !StringUtils.hasText(job) && !StringUtils.hasText(location)){
            jobPost = jobPostActivityService.getAll();
        } else {
            // if any of the flags are selected, search using these flags
        ...
        if(!(authentication instanceof AnonymousAuthenticationToken)){
            ...
            if(authentication.getAuthorities().contains(new SimpleGrantedAuthority("Recruiter"))){
                ...
                //a list of jobs that a candidate has saved already in the past
                List<JobSeekerSave> jobSeekerSaveList = jobSeekerSaveService.getCandidatesJob((JobSeekerProfile) currentUserProfile);

                boolean exist;
                boolean saved;

                //loop through the job posts and check if this is a job that we applied for or just saved it in the past
